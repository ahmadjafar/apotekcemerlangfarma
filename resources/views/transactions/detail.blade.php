<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Transaction &raquo; {{ $item->drug->name ?? "Data Tidak Ditemukan" }} by {{ $item->user->name ?? "Data Tidak Ditemukan" }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="w-full rounded overflow-hidden shadow-lg px-6 py-6 bg-white">
                <div class="flex -mx-4 -mb-4 md:mb-0">
                        <div class="w-full md:w-1/6 px-2 mb-4 md:mb-0">
                            <img src="{{ $item->drug->picturePath ?? "Data Tidak Ditemukan"}}" alt="" class="w-full rounded">
                        </div>
                    <div class="w-full md:w5/6 px-4 mb-4 md:mb-0">
                        <div class="flex flex-wrap mb-3">
                            <div class="w-2/6">
                                <div class="text-sm">Product Name</div>
                                <div class="text-xl font-bold">{{ $item->drug->name ?? "Data Tidak Ditemukan"}}</div>
                            </div>
                            <div class="w-1/6">
